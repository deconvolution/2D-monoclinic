...
xc=150;
zc=150;
[tt2,tt]=meshgrid(1:size(A,1),1:size(A,2));
tt=reshape(tt,[size(A,1)*size(A,2),1]);
tt2=reshape(tt2,[size(A,1)*size(A,2),1]);
tt3=find((tt-xc).^2+(tt2-zc).^2>10^3);
tt(tt3)=[];
tt2(tt3)=[];
A(sub2ind(size(A),tt2,tt))=0;
